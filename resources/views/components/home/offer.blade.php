 <section class="offer-section" id="about">
        <div class="container">
            <h2>What Can We Offer?</h2>
            <p class="section-subtitle">At our tuition center, we offer comprehensive IT tuition services tailored to
                your needs. We're committed to empowering you to excel in the ever-evolving world of technology.</p>
            <div class="offer-grid">
                <div class="offer-item"> <i class="fas fa-code"></i>
                    <h4>Programming Languages</h4>
                    <p>Learn popular languages like Python, Java, C#, and JavaScript from expert instructors.</p>
                </div>
                <div class="offer-item"> <i class="fas fa-laptop-code"></i>
                    <h4>Software Development</h4>
                    <p>Master the principles of software engineering and build robust applications.</p>
                </div>
                <div class="offer-item"> <i class="fas fa-database"></i>
                    <h4>Database Management</h4>
                    <p>Gain expertise in SQL, NoSQL, and database administration for efficient data handling.</p>
                </div>
                <div class="offer-item"> <i class="fas fa-globe"></i>
                    <h4>Web Development</h4>
                    <p>Create dynamic and responsive websites using front-end and back-end technologies.</p>
                </div>
                <div class="offer-item"> <i class="fas fa-chart-line"></i>
                    <h4>Skill Enhancement & Career Growth</h4>
                    <p>From beginners to seasoned pros, we help you kickstart your journey or advance your skills.</p>
                </div>
                <div class="offer-item"> <i class="fas fa-user-graduate"></i>
                    <h4>Hands-on & Personalized Learning</h4>
                    <p>Benefit from practical training, individual attention, and flexible scheduling options.</p>
                </div>
            </div> <button class="btn-primary" onclick="scrollToCourses()">Explore Courses</button>
        </div>
    </section> 